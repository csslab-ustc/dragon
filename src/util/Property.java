...
public class Property<X, Y> {
    ...
    // if the property is not present, return null
    ...
    public Y getOrInitConst(X element, Y constantValue) {
        Plist plist = getPlist.apply(element);
        Object value = plist.get(this);
        if (value != null)
            return (Y) value;
    ...
    public Y getOrInitFun(X element, Function<X, Y> fun) {
        Plist plist = getPlist.apply(element);
        Object value = plist.get(this);
        if (value == null) {
